...
- **Hermes Secrets** - dashboard-username, dashboard-password, dashboard-session-secret (the dashboard's bundled `basic` auth provider — the 0.0.0.0 bind is fail-closed and will not start without a registered provider, so all three fields must exist. dashboard-session-secret is a >=32-byte random value, `openssl rand -hex 32`, so dashboard logins survive pod restarts). No LLM-provider API key here: Hermes' LLM engine is the Codex app-server runtime, authenticated by a **one-time** `codex login` (ChatGPT-subscription OAuth — subscription billing, not API tokens), token persisted in `CODEX_HOME=/opt/data/.codex/auth.json` on the encrypted NFS volume, not in 1Password (`task hermes:codex-login`; docs/37 §LLM engine). Additional Hermes provider/platform keys are entered in the dashboard UI (persisted to the encrypted, backed-up `/opt/data/.env`), not here
