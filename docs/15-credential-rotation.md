...
- **Hermes Secrets** - dashboard-username, dashboard-password, dashboard-session-secret, api-server-key (the dashboard's bundled `basic` auth provider — the 0.0.0.0 bind is fail-closed and will not start without a registered provider — plus the bearer key for the gateway's in-process OpenAI-compatible API server, enabled as the gateway's health-probe surface; the server refuses to start without it. All four fields must exist — a missing field fails the whole ESO Secret sync. dashboard-session-secret and api-server-key are >=32-byte random values, `openssl rand -hex 32`). No LLM-provider API key here: Hermes' LLM engine is the Codex app-server runtime, authenticated by a **one-time** `codex login` (ChatGPT-subscription OAuth — subscription billing, not API tokens), token persisted in `CODEX_HOME=/opt/data/.codex/auth.json` on the encrypted NFS volume, not in 1Password (`task hermes:codex-login`; docs/37 §LLM engine). Additional Hermes provider/platform keys are entered in the dashboard UI (persisted to the encrypted, backed-up `/opt/data/.env`), not here
