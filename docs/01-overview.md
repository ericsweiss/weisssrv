...
Internet
    |
[Gateway/Firewall] - UniFi UCG-Fiber (10.0.10.1 on VLAN 10; 10.0.1.1 on VLAN 1)
    |
[USW-Pro-XG-8-PoE] --- trunk (native VLAN 1, tagged 10/20/30/40/50)
    |                       |
    |                       +-- [U7 Pro XGS] --- wireless: Home/IoT/Guest/Work
    |
[VLAN 10 - 10.0.10.0/24] ---- Homelab (hosts, guests, k3s, VIPs)
    |
    +-- Proxmox Cluster (6 nodes, cluster name: weisssrv)
    |   +-- pve-nas-01    (10.0.10.102) - NAS + Storage
    |   +-- pve-laptop-01 (10.0.10.103) - Compute
    |   +-- pve-opt-01    (10.0.10.104) - Compute
    |   +-- pve-opt-02    (10.0.10.105) - Compute
    |   +-- pve-opt-03    (10.0.10.106) - Compute
    |   +-- pve-prec-01   (10.0.10.107) - Compute (Dell Precision 3630, 62G RAM, GTX 1660 Ti → VM 207 via VFIO; docs/43)
    |
    +-- Infrastructure LXC (HA-managed)
    |   +-- dns-01        (10.0.10.150) - Primary DNS (AdGuard + Unbound)
    |   +-- dns-02        (10.0.10.160) - Secondary DNS (AdGuard + Unbound)
    |   +-- smtp-relay    (10.0.10.151) - Mail relay (Postfix -> Gmail)
    |
    +-- Application LXC
    |   +-- plex          (10.0.10.152) - Plex Media Server (plex.esweiss.com)
    |   +-- immich-ml     (10.0.10.158) - Immich GPU ML (OpenVINO, Arc B580 /dev/dri shared with plex; docs/36)
    |
    +-- Application VMs
    |   +-- gitlab        (10.0.10.153) - GitLab EE (git.esweiss.com)
    |   +-- home-assistant(10.0.10.154) - Home Assistant OS (HA-managed)
    |   +-- windows       (10.0.10.155) - Windows 11 VM (IaC-provisioned shell, interactive install; docs/39)
    |   +-- nextcloud     (10.0.10.156) - Nextcloud (cloud.esweiss.com; docs/35)
    |   +-- immich        (10.0.10.157) - Immich (photos.esweiss.com; docs/36)
    |
    +-- K3s Cluster VMs (9 nodes: 3 servers + 6 agents)
    |   +-- Servers (.22X range - control plane + etcd)
    |   |   +-- k3s-srv-nas-01    (10.0.10.222) - Server + etcd
    |   |   +-- k3s-srv-laptop-01 (10.0.10.223) - Server + etcd
    |   |   +-- k3s-srv-prec-01   (10.0.10.227) - Server + etcd
    |   +-- Agents (.20X range - workers)
    |       +-- k3s-agt-nas-01    (10.0.10.202) - Agent (NAS workloads)
    |       +-- k3s-agt-laptop-01 (10.0.10.203) - Agent (ingress + general)
    |       +-- k3s-agt-opt-01    (10.0.10.204) - Agent (ingress + general)
    |       +-- k3s-agt-opt-02    (10.0.10.205) - Agent (ingress + general)
    |       +-- k3s-agt-opt-03    (10.0.10.206) - Agent (ingress + general)
    |       +-- k3s-agt-prec-01   (10.0.10.207) - Agent (general + compute + GPU; 30G, 1660 Ti for Hindsight; docs/43)
    |
    +-- Virtual IPs
...
| 10 | Homelab | 10.0.10.0/24 | 10.0.10.1 | .2-.98 | Everything in the tree above |
...
|-------|---------|--------|
| 10.0.10.1-98 | Infrastructure, DHCP, workstations | Active |
...
storage. Mind the CIDR gotcha: `10.0.10.220/29` masks to .216–.223 and does
**not** cover server .227 (prec-01), so every export that needs .227 carries an
explicit `10.0.10.227/32` line alongside the two /29 blocks — the RW exports
