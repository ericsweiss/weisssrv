...
| Immich VM (.157) → Immich ML LXC (.158) :3003 | No | Every photo byte crosses the LAN as plain HTTP. Scoped to the one source by the `sg-immich-ml` security group (`IN ACCEPT -source 10.0.10.157 -p tcp -dport 3003`). See docs/36. |
