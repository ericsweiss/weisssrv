...
Then: re-vendor every file this repo's own manifest lists. The manifest is
`scripts/vendored-manifest.yml` and it covers more than `scripts/` (the shared
lint profiles at the repo root are in it too) — never work from a remembered
list:

```bash
python3 ../weisssrv-lib/scripts/check-vendored-copies.py \
  --repo-root . --list                       # what to copy
python3 ../weisssrv-lib/scripts/check-vendored-copies.py \
  --repo-root . --ref <new-tag>              # what still drifts
```
